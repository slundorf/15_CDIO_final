...
public class BusinessLayerImplementation implements IBusinessLayer, IRoleDAO {
	...
	public BusinessLayerImplementation(IUserDAO userDAO, IRoleDAO roleDAO, IIngredientDAO ingredientDAO,
			IIngredientBatchDAO ingredientBatchDAO, IRecipeDAO recipeDAO, IProductBatchDAO productBatchDAO,
			IProductBatchComponentDAO productBatchComponentDAO, IRecipeComponentDAO recipeComponentDAO) {
	...
	private void checkUser(UserDTO user) throws DALException {
		...
		/*
		 * Check Initials tempIni.matches(".*\\d+.*")
		 * ---------------------------- To explain: .* means any character from
		 * 0 to infinite occurence, than the \\d+ (double backslash I think is
		 * just to escape the second backslash) and \d+ means a digit from 1
		 * time to infinite.
		 */
	 ...
	 * @throws DALException
	 */
	private boolean checkCpr(UserDTO user) throws DALException {
		Date date = null;
		// First try and catch for "-" error
		for (int i = 0; i < userDAO.getUserList().size(); i++) {
			if (userDAO.getUserList().get(i).getUserID()!=user.getUserID() && userDAO.getUserList().get(i).getCpr().equals(user.getCpr())) {
				throw new DALException("Another user has the CPR: " + user.getCpr());
			}
		}
		try {
			String[] parts = user.getCpr().split("-");
	...
	public void createIngredientBatch(IngredientBatchDTO ingredientBatch) throws DALException {
		for (int i = 0; i < ingredientBatchDAO.getIngredientBatchList().size(); i++) {
			if (ingredientBatchDAO.getIngredientBatchList().get(i).getIngredientBatchID() == ingredientBatch
					.getIngredientBatchID()) {
		...
		for (int i = 0; i < recipeComponentDAO.getRecipeComponentList().size(); i++) {
			if (recipeComponentDAO.getRecipeComponentList().get(i).getRecipeID() == recipeComponent.getRecipeID()
					&& recipeComponentDAO.getRecipeComponentList().get(i).getRecipeID() == recipeComponent
							.getIngredientID()) {
				throw new DALException("Recipe " + recipeComponent.getRecipeID() + " already has ingredient "
						+ recipeComponent.getIngredientID() + " in a component.");
			}
		}
